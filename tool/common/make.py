...
def include(name, dir=None, file=None):
    if dir is not None:
        shutil.copytree(dir,  f"./binary/{type}/package/{name}/install/include", dirs_exist_ok=True)
    if file is not None:
        os.makedirs          (f"./binary/{type}/package/{name}/install/include", exist_ok=True)
        shutil.copyfile(file, f"./binary/{type}/package/{name}/install/include/{os.path.basename(file)}")
...
def src(name, dir, src_to_include=None):
    ...
    shutil.copytree(dir, f"./binary/{type}/package/{name}/install/include/{src_to_include}", dirs_exist_ok=True)
...
def cmake(name, dir, args=[]):
    run(f"cmake -S ./{dir} "
            f"      -B ./binary/{type}/package/{name}/build "
            ...
            f"      -DCMAKE_INSTALL_PREFIX=./binary/{type}/package/{name}/install "
            f"      -DCMAKE_BUILD_TYPE={type} "
            f"{' '.join(args)}")
    run(f"cmake --build   ./binary/{type}/package/{name}/build -j {os.cpu_count()}", quiet=True)
    run(f"cmake --install ./binary/{type}/package/{name}/build -j {os.cpu_count()}")
...
def configure(name, file, args=[]):
    if not Package(name).is_configured:
        cwd = f"./binary/{type}/package/{name}/build"
        try:
            os.makedirs(cwd, exist_ok=True)
            run(f"./{os.path.relpath(file, cwd)} --prefix={os.path.abspath(f"./binary/{type}/package/{name}/install")} {' '.join(args)}", cwd=cwd)
        except:
            shutil.rmtree(cwd)
            raise
