...
namespace ap
{
    ...
    constexpr array_type     auto   operator -     (                               const array_type auto&  right ) requires requires {                                 -  std::declval<right_value_type>(); };
    constexpr array_type     auto   operator +     ( const array_type auto&  left, const array_type auto&  right ) requires requires { std::declval<left_value_type>() +  std::declval<right_value_type>(); }                                     and ( left_dimension == right_dimension )            and same_as<left_device_type,right_device_type>;
    constexpr array_type     auto   operator -     ( const array_type auto&  left, const array_type auto&  right ) requires requires { std::declval<left_value_type>() -  std::declval<right_value_type>(); }                                     and ( left_dimension == right_dimension )            and same_as<left_device_type,right_device_type>;
    constexpr array_type     auto   operator *     ( const array_type auto&  left, const array_type auto&  right ) requires requires { std::declval<left_value_type>() *  std::declval<right_value_type>(); }                                     and ( left_dimension == 2 and right_dimension == 2 ) and same_as<left_device_type,right_device_type>;
    constexpr array_type     auto   operator *     ( const array_type auto&  left, const            auto&  right ) requires requires { std::declval<left_value_type>() *  std::declval<right_type      >(); } and ( not array_type<right_type> );
    constexpr array_type     auto   operator *     ( const            auto&  left, const array_type auto&  right ) requires requires { std::declval<left_type      >() *  std::declval<right_value_type>(); } and ( not array_type<left_type > );
    constexpr array_type     auto   operator /     ( const array_type auto&  left, const            auto&  right ) requires requires { std::declval<left_value_type>() /  std::declval<right_type      >(); } and ( not array_type<right_type> );
    constexpr array_type     auto&  operator +=    (       array_type auto&  left, const array_type auto&  right ) requires requires { std::declval<left_value_type>() += std::declval<right_value_type>(); }                                                                                          and same_as<left_device_type,right_device_type>;
    constexpr array_type     auto&  operator -=    (       array_type auto&  left, const array_type auto&  right ) requires requires { std::declval<left_value_type>() -= std::declval<right_value_type>(); }                                                                                          and same_as<left_device_type,right_device_type>;
    constexpr array_type     auto&  operator *=    (       array_type auto&  left, const            auto&  right ) requires requires { std::declval<left_value_type>() *= std::declval<right_value_type>(); }                                                                                          and same_as<left_device_type,right_device_type>;
    constexpr array_type     auto&  operator /=    (       array_type auto&  left, const            auto&  right ) requires requires { std::declval<left_value_type>() /= std::declval<right_value_type>(); }                                                                                          and same_as<left_device_type,right_device_type>;
