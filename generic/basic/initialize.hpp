...
namespace detail
{
    struct basic_initializer_t  
    {
        ...
        void basic_initializer_t::abort_signal_handler ( int )
        {
            throw ap::abort_signal("SIGABRT (process abort signal)");
        }

        void basic_initializer_t::floating_point_exception_signal_handler ( int )
        {
            throw ap::floating_point_exception_signal("SIGFPE (erroneous arithmetic operation)");
        }

        void basic_initializer_t::illegal_instruction_signal_handler ( int )
        {
            throw ap::illegal_instruction_signal("SIGILL (illegal instruction)");      
        }

        void basic_initializer_t::interrupt_signal_handler ( int )
        {
            throw ap::interrupt_signal("SIGINT (terminal interrupt signal)");                
        }

        void basic_initializer_t::segmentation_violation_signal_handler ( int )
        {
            throw ap::segmentation_violation_signal("SIGSEGV (invalid memory reference)");   
        }

        void basic_initializer_t::terminate_signal_handler ( int )
        {
