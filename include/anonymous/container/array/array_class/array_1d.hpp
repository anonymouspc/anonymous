...
class array<type,1,device>
    extends private device::template vector<type>,                 // Make private to prevent unsafe static_cast.
            ...
            public  array_algo<array<type,1,device>,type,1,device>
{
    ...
    public: // Conversion
        ...
        constexpr          array ( const array<type,1,device2>& )       requires same_as
        constexpr          array ( const array_type auto& other )       requires convertible_to<other_value_type,value_type> and ( other.dimension() == dim ) and ( same_as<device,cpu> or same_as<other_device_type,cpu> );
