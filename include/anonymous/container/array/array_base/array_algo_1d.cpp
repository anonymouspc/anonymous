...
{
    contiguous() ? (void) device::reverse(data(), data() + size()) :
                   (void) device::reverse(begin(), end());
    return static_cast<container&>(self);
}
...
constexpr container& array_algo<container,type,1,device>::rotate ( int n )
{
    if constexpr ( debug )
        if ( n <= -size() or n >= size() )
            throw value_error("cannot rotate array (with step = {}, size() = {}): step out of range", n, size());
...
constexpr container& array_algo<container,type,1,device>::stable_sort ( )
    requires comparable<type>
{
    contiguous() ? (void) device::stable_sort(data(), data() + size()) : 
                   (void) device::stable_sort(begin(), end());
    return static_cast<container&>(self);
}
...
{
    contiguous() ? (void) device::stable_sort(data(), data() + size(), pred) : 
                   (void) device::stable_sort(begin(), end(), pred);
    return static_cast<container&>(self);
}
...
constexpr container& array_algo<container,type,1,device>::sort ( )
    requires comparable<type>
{
    contiguous() ? (void) device::sort(data(), data() + size()) : 
                   (void) device::sort(begin(), end());
    return static_cast<container&>(self);
}
...
{    
    contiguous() ? (void) device::sort(data(), data() + size(), pred) : 
                   (void) device::sort(begin(), end(), pred);
    return static_cast<container&>(self);
    ...
    requires default_initializable<type> and plusable<type> and dividable_to<type,int>
{
    if constexpr ( debug )
        if ( empty() )
            throw value_error("cannot calculate average (with empty() = true): array is empty");
