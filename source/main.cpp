...
{
   elements::app _app("TextEdit");
   elements::window _win("TextEdit");
   _win.on_close = [&_app]() { _app.stop(); };

   elements::view view_(_win);

   view_.content(
      elements::scroller(
         elements::margin(
            {20, 20, 20, 20},
            elements::align_left_top(elements::hsize(800,
               elements::basic_text_box(text1+text2+text3, elements::font_descr{"menlo", 14}
            )))
         )
      ),
      elements::port(elements::image{"/Users/anonymous/Code/anonymous/space.jpg"})
   );
