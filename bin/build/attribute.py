        ...
        for attr   in   re.findall(match_attr,       attr_quote,     flags=re.DOTALL):
            ...
            if attr_key.startswith("anonymous::"):
                if attr_key == "anonymous::cmake_directory":
                    _cmake_directory(dir =attr_vals[0], args=attr_vals[1:])
                elif attr_key == "anonymous::make_directory":
                    _make_directory (dir =attr_vals[0], args=attr_vals[1:])
                elif attr_key == "anonymous::autogen_file":
                    _autogen_file   (file=attr_vals[0], args=attr_vals[1:])
                elif attr_key == "anonymous::configure_file":
                    _configure_file (file=attr_vals[0], args=attr_vals[1:])
                elif attr_key == "anonymous::perl_file":
                    _perl_file      (file=attr_vals[0], args=attr_vals[1:])
                elif attr_key == "anonymous::update_header":
                    _update_header  (file=attr_vals[0], args=attr_vals[1:])
                elif attr_key == "anonymous::update_library":
                    _update_library (file=attr_vals[0], args=attr_vals[1:])
                else:
...
def _run_command(module, command, **kwargs):
...
def _cmake_directory(module, dir, args):
    try:
        os.mkdir(f"./bin/{type}/cmake/{module}-build")
        os.mkdir(f"./bin/{type}/cmake/{module}-install")
    except:
        pass
    _run_command(f"cmake -S {dir} -B ./bin/{type}/cmake/{module}-build "
                 f"--install-prefix={os.path.abspath(f"./bin/{type}/cmake/{module}-install")} "
                 f"-DCMAKE_C_COMPILER={c_compiler} "
                 f'-DCMAKE_C_FLAGS="{' '.join(c_compile_args)} '
                 f"-DCMAKE_CXX_COMPILER={compiler} "
                 f'-DCMAKE_CXX_FLAGS="{' '.join(compile_args)}'
                 f"-DCMAKE_BUILD_TYPE={type.capitalize()}")
    _run_command(f"cmake --build   ./bin/{type}/cmake/{module}-build")
    _run_command(f"cmake --install ./bin/{type}/cmake/{module}-build")
...
def _make_directory(module, dir, args):
    try:
        ...
        pass
    _run_command(f"make",         cwd=f"./bin/{type}/cmake/{module}-build")
    _run_command(f"make install", cwd=f"./bin/{type}/cmake/{module}-build")
...
def _configure_file(module, file, args):
    try:
        ...
        pass
    _run_command(f"sh {os.path.abspath(file)} --prefix={os.path.abspath(f"./bin/{type}/cmake/{module}-install")}", 
                 cwd=f"./bin/{type}/cmake/{module}-build",
                 env=os.environ + [f"CC={c_compiler}", f"CFLAGS={c_compile_args}", f"CXX={compiler}", f"CXXFLAGS={' '.join(compile_args)}"])
...
def _perl_file(module, file, args):
    try:
        ...
        pass
    _run_command(f"perl {os.path.abspath(file)} --prefix={os.path.abspath(f"./bin/{type}/cmake/{module}-install")} --{type}", cwd=f"./bin/{type}/cmake/{module}-build")
...
def _update_header(module, file, args):
    ...
    try:
        shutil.copyfile(f"./bin/{type}/cmake/{module}-install/include/{file.strip("<>")}", f"./third_party/include/{file.strip("<>")}")
...
def _update_library(module, file, args):
    try:
        if '.' in file:
            shutil.copyfile(f"./bin/{type}/cmake/{module}-install/lib/{file}",                 f"./bin/{type}/module/{file}"                )
        else:
            shutil.copyfile(f"./bin/{type}/cmake/{module}-install/lib/{file}{library_suffix}", f"./bin/{type}/module/{file}{library_suffix}")
