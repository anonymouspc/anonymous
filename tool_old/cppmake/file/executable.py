from cppmake.common.compiler import run_executable
from cppmake.common.config   import argv, executable_suffix
...
class Executable:
    async def __new__(self, name):
        if name in Executable.pool.keys():
            ...
            self.new_task     = asyncio.create_task(self._new_task(name=name))
            self.compile_task = asyncio.create_task(self._compile_task())
