...
export namespace anonymous
{
    ...
    {
        eigen_map(output).noalias() = + eigen_map<output_value_type>(right);
    }
    ...
    {
        eigen_map(output).noalias() = eigen_map<output_value_type>(left) + eigen_map<output_value_type>(right);
    }
    ...
    {
        eigen_map(output).noalias() = eigen_map<output_value_type>(left) * right;
    }
    ...
    {
        eigen_map(left) += eigen_map<left_value_type>(right);
    }
    ...
    {
        eigen_map<output_value_type,eigen_map_tensor>(output) = eigen_map<output_value_type,eigen_map_tensor>(left).convolve(eigen_map<output_value_type,eigen_map_tensor>(right), eigen_make_convolve_full_dims<left.rank()>());
    }

    constexpr void cpu::linalg::cross ( const auto left, const auto right, auto output )
    {
        eigen_map(output).noalias() = eigen_map<output_value_type>(left).cross(eigen_map<output_value_type>(right));
    }

    constexpr void cpu::linalg::dot ( const auto left, const auto right, auto output )
    {
        eigen_map(output).noalias() = eigen_map<output_value_type>(left).dot(eigen_map<output_value_type>(right));
    }

    constexpr void cpu::linalg::fft ( const auto vector, auto output )
    {
        auto output_map = eigen_map(output);
        Eigen::FFT<output_value_type::value_type>().fwd(output_map, eigen_map(vector));
    }

    constexpr void cpu::linalg::ifft ( const auto vector, auto output )
    {
        auto output_map = eigen_map(output);
        Eigen::FFT<output_value_type::value_type>().inv(output_map, eigen_map(vector));
    }

    constexpr void cpu::linalg::tensor ( const auto left, const auto right, auto output )
    {
        eigen_map(output).noalias() = eigen_map<output_value_type>(left) * eigen_map<output_value_type>(right).transpose();
    }
}