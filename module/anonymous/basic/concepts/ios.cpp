...
export namespace anonymous
{
    ...
    template < class type > concept async_reader = requires ( type& r, char* c, int s ) { { r.async_read (c, s) } -> std::execution::sender_of<std::execution::set_value_t(int)>; };
    template < class type > concept async_writer = requires ( type& r, char* c, int s ) { { r.async_write(c, s) } -> std::execution::sender_of<std::execution::set_value_t(int)>; };
