...
export namespace anonymous
{
    ...
    template < class type > concept async_reader = requires ( type& r, char* c, int s ) { { r.async_read (c, s) } -> convertible_to<async<int>>; };
    template < class type > concept async_writer = requires ( type& r, char* c, int s ) { { r.async_write(c, s) } -> convertible_to<async<int>>; };
