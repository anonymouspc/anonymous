    ...
    extends public std::tuple<types...>
{
    ...
    public: // Core
        constexpr tuple ( )                                                      requires ( std::default_initializable<remove_cv<types>> and ... ) = default;
        constexpr tuple ( types... );

    public: // Conversion
        template < class... types2 > constexpr tuple ( const tuple<types2...>& ) requires ( ( std::convertible_to<types2,types>     and ... ) but ( ( not std::same_as<types,types2>        ) or ... ) );
        template < class... types2 > constexpr tuple ( const tuple<types2...>& ) requires ( ( std::constructible_from<types,types2> and ... ) but ( ( not std::convertible_to<types2,types> ) or ... ) );
