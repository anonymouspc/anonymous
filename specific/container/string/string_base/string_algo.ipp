...
constexpr container& string_algo<container,type,device>::format ( auto&&... f )
    requires ( ( std::formattable<decay<decltype(f)>,char> or constructible_from<container,decay<decltype(f)>> ) and ... )
{
    try
    {
        return detail::get_format_mode(data()) == detail::explicit_mode ?
                   static_cast<container&>(self) = std::format(std::runtime_format(std::string("{0}") + data()), "", detail::make_formattable(f)...) otherwise
                   static_cast<container&>(self) = std::format(std::runtime_format(                     data()),     detail::make_formattable(f)...);
    }
...
// {
//     return derive_of_self = container ( std::regex_replace ( std::basic_string<char_type>(derive_of_self), std::basic_regex<char_type>(rgx), std::basic_string<char_type>() ) );
// }

// template < class container, class type, class device >
// constexpr container& string_algo<container,type,device>::replace ( const regex& rgx, const general_string_type auto& str )
//     requires ( not is_view )
// {
//     return derive_of_self = container ( std::regex_replace ( std::basic_string<char_type>(derive_of_self), std::basic_regex<char_type>(rgx), std::basic_string<char_type>(str) ) );
// }
