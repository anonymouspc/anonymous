...
//         executor_with_default ( const auto& ex ) noexcept requires ( convertible_to<decay<decltype(ex)>,executor_type> but not same_as<decay<decltype(ex)>,executor_with_default> ) ;
// };
...
use_sender_t::executor_with_default<executor_type>::executor_with_default ( const auto& ex ) noexcept
    ...
    /* Note: this requires-clause should explicitly remain first not same_as<ex,self>, then convertible_to<lower_executor>,
     * as convertible_to<executor_with_default,lower_executor> depends on itself.
     */
    extends executor_type ( ex )
{
